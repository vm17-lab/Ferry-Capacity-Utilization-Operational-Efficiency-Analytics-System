...
def load_processed_data():
    ...
    if not os.path.exists(path):
        ...
        df = pd.DataFrame({
            'Timestamp': date_rng,
            'OLI': np.random.uniform(0.2, 0.9, size=len(date_rng)),
            'Season': np.random.choice(["Peak Season", "Shoulder", "Off-Season"], size=len(date_rng)),
            'Redemption_Pressure_Ratio': np.random.uniform(0.5, 2.5, size=len(date_rng)),
            'Total_Activity_Load': np.random.randint(50, 500, size=len(date_rng)),
            'Idle_Capacity_Indicator': np.random.choice([True, False], size=len(date_rng))
        })
    else:
    ...
    df = df[df['Timestamp'].dt.month.isin(range(5, 13))]
...
if not df.empty:
    ...
    if fdf.empty:
        ...
        pressure = fdf["Redemption_Pressure_Ratio"].mean() if "Redemption_Pressure_Ratio" in fdf.columns else 0
        total_activity = fdf["Total_Activity_Load"].sum() if "Total_Activity_Load" in fdf.columns else 0
        stability = 100 - (fdf["Utilization"].std()) if len(fdf) > 1 else 100
        idle_pct = (fdf[
                        "Idle_Capacity_Indicator"] == True).mean() * 100 if "Idle_Capacity_Indicator" in fdf.columns else 0
        ...
        cols = st.columns(5)
        with cols[0]:
            st.markdown(kpi_card("⚖️", "Avg Utilization", f"{avg_util:.1f}%", "EFFICIENCY", "#00e5ff"),
                        unsafe_allow_html=True)
        with cols[1]:
            st.markdown(kpi_card("🔺", "Pressure Index", f"{pressure:.2f}", "DEMAND", "#ff4d6d"), unsafe_allow_html=True)
        with cols[2]:
            st.markdown(kpi_card("💤", "Idle Capacity", f"{idle_pct:.1f}%", "WASTE RATIO", "#ffb347"),
                        unsafe_allow_html=True)
        with cols[3]:
            st.markdown(kpi_card("⏱️", "Total Load", f"{int(total_activity):,}", "VOLUME", "#c084fc"),
                        unsafe_allow_html=True)
        with cols[4]:
            st.markdown(kpi_card("📐", "Stability", f"{max(0, stability):.1f}", "CONSISTENCY", "#00ffa3"),
                        unsafe_allow_html=True)
        ...
        with c1:
            st.markdown(f'<div class="sec-title">📈 {freq_label.upper()} UTILIZATION TREND</div>',
                        unsafe_allow_html=True)
            ts_data = fdf.set_index("Timestamp").resample(FREQ_MAP[freq_label]).agg(
        ...
        with c2:
            ...
            fig_vio = px.violin(fdf, y="Utilization", x="Season", color="Season", color_discrete_map=SEASON_COLORS,
                                box=True)
            apply_base(fig_vio)
        ...
        fdf['Bucket'] = fdf['Timestamp'].dt.round('15min').dt.strftime('%H:%M')
        hm_data = fdf.groupby(["Day", "Bucket"])["Utilization"].mean().unstack().reindex(day_order).fillna(0)
        ...
        if cong_pct > 15:
            peak_time = fdf.loc[fdf['Utilization'].idxmax()]
            st.markdown(f"""
            <div class="alert-box alert-critical">
                <strong>CRITICAL CAPACITY ALERT:</strong> High congestion detected in <b>{cong_pct:.1f}%</b> of monitored time slots.<br>
                Peak load identified on <b>{peak_time['Timestamp'].strftime('%Y-%m-%d %H:%M')}</b> reaching <b>{peak_time['Utilization']:.1f}%</b> capacity.
            </div>
            """, unsafe_allow_html=True)
        else:
            st.markdown(f"""
            <div class="alert-box alert-nominal">
                <strong>SYSTEM NOMINAL:</strong> Congestion threshold ({threshold}%) only exceeded in <b>{cong_pct:.1f}%</b> of monitored periods.<br>
                Operation efficiency is within target parameters.
            </div>
            """, unsafe_allow_html=True)